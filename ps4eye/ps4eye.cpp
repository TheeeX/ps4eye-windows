...
#define ARRAY_SIZE(_A) (sizeof(_A) / sizeof((_A)[0]))
	...
	static int64_t getTickCount() {
		...
		auto value = timeMS.time_since_epoch();
		int64_t t = value.count();
		return t;
...
#define UVC_STREAM_FID	(1 << 0)
	...
	public:
		URBDesc(UINT mode) :
			...
			frame_buffer = (uint8_t*)malloc(
				frame_size * 16
				+ ISO_PACKET_SIZE * ISO_PACKETS_PER_TRANSFER * 15);
			if (!frame_buffer) {
				debug("error reservando\n");
			}
			//		memset(frame_buffer, 0, frame_size * 16 + 5571968); //ISO_PACKET_SIZE*8*15);
			memset(frame_buffer, 0,
				frame_size * 16
				+ ISO_PACKET_SIZE * ISO_PACKETS_PER_TRANSFER * 15); //ISO_PACKET_SIZE*8*15);
		...
		URBDesc() :
			...
			frame_buffer = (uint8_t*)malloc(
				fsz * 16 + ISO_PACKET_SIZE * ISO_PACKETS_PER_TRANSFER * 15);
			if (!frame_buffer) {
				debug("error reservando\n");
			}
			memset(frame_buffer, 0, fsz * 16 + 5571968); //ISO_PACKET_SIZE*8*15);
		...
		bool start_transfers(libusb_device_handle *handle,
			...
			for (i = 0; i < num_transfers; i++) {
				xfr[i] = libusb_alloc_transfer(ISO_PACKETS_PER_TRANSFER);
				...
				xfr[i]->timeout = 10000;
				xfr[i]->buffer = frame_buffer_end+ISO_PACKET_SIZE*8*i;
				xfr[i]->user_data = this;
				xfr[i]->length = ISO_PACKET_SIZE*8; // max size in transfer ISO_PACKET_SIZE*8
				xfr[i]->callback = cb_xfr;
				xfr[i]->num_iso_packets = 8; //num isoc*/
				libusb_fill_iso_transfer(xfr[i], handle, 0x81,
					frame_buffer_end
					+ ISO_PACKET_SIZE * ISO_PACKETS_PER_TRANSFER * i,
					ISO_PACKET_SIZE * ISO_PACKETS_PER_TRANSFER,
					ISO_PACKETS_PER_TRANSFER, cb_xfr,
					reinterpret_cast<void*>(this), 1000);
				libusb_set_iso_packet_lengths(xfr[i], ISO_PACKET_SIZE);
				//			xfr[i]->flags = LIBUSB_TRANSFER_ADD_ZERO_PACKET;
		...
		void pkt_scan(uint8_t *data, int len) {
			...
			payload_len = ISO_PACKET_SIZE; // ISO_PACKET_SIZE; //ISO_PACKET_SIZE*8; // isoc superspeed usb transfer
			...
			do {
				...
				if (!(data[1] & UVC_STREAM_PTS)) {
					...
					//std::cout << "PTS not present" << std::endl;

					goto discard;
				}

				this_pts = (data[5] << 24) | (data[4] << 16) | (data[3] << 8)
					| data[2];
				...
				if (this_pts != last_pts || this_fid != last_fid) {
					if (last_packet_type == INTER_PACKET) {
						//cout << frame_data_len << endl;
						frame_add(LAST_PACKET, NULL, 0);
					}
					last_pts = this_pts;
					last_fid = this_fid;
					frame_add(FIRST_PACKET, data + header_length, len - header_length);
				} /* If this packet is marked as EOF, end the frame */
				else if (data[1] & UVC_STREAM_EOF) {
					last_pts = 0;
					if (frame_data_len + len - header_length != frame_size) {
						//cout << frame_data_len + len - header_length << endl;
						goto discard;
					}
					frame_add(LAST_PACKET, data + header_length, len - header_length);
				}
				else {
					/* Add the data from this payload */
					frame_add(INTER_PACKET, data + header_length, len - header_length);
				}
		...
		if (xfr->type == LIBUSB_TRANSFER_TYPE_ISOCHRONOUS) {
			...
			for (i = 0; i < xfr->num_iso_packets; i++) {
				...
				if (pack->status != LIBUSB_TRANSFER_COMPLETED) {
					...
					//std::cout << pack->actual_length << std::endl;
					if (pack->actual_length > 0) {
						//std::cout << pack->actual_length << std::endl;
						urb->pkt_scan(xfr->buffer + ISO_PACKET_SIZE * i,
							pack->actual_length);
					}
				}

				//			if (pack->actual_length > 0) {
				//				urb->pkt_scan(xfr->buffer + ISO_PACKET_SIZE * i, pack->actual_length);
				//			}
	...
	const std::vector<PS4EYECam::PS4EYERef>& PS4EYECam::getDevices(
		bool forceRefresh) {
		...
		USBMgr::instance()->sTotalDevices = USBMgr::instance()->listDevices(
			devices);
	...
	*/
	uint8_t PS4EYECam::register_read(uint16_t reg, uint8_t subaddr) {
		...
		submitAndWait_controlTransfer(
			LIBUSB_ENDPOINT_OUT | LIBUSB_REQUEST_TYPE_VENDOR
			| LIBUSB_RECIPIENT_DEVICE, 0xa5, 0, 0, 0X40, buffer_out);
		...
		submitAndWait_controlTransfer(
			...
			| LIBUSB_RECIPIENT_DEVICE, 0xa6, 0, 0, 0X40, buffer_in);
		/*  for(i=0;i<20;i++)
		{
	...
	*/
	void PS4EYECam::set_sensor_video_mode(uint8_t mode, uint8_t n) {
		...
		switch (mode) {
		case 0:
			multi_register_write(ov9713_video_mode_0,
				ARRAY_SIZE(ov9713_video_mode_0), n);
			break;
		case 1:
			multi_register_write(ov9713_video_mode_1,
				ARRAY_SIZE(ov9713_video_mode_1), n);
			break;
		case 2:
			multi_register_write(ov9713_video_mode_2,
				ARRAY_SIZE(ov9713_video_mode_2), n);
			break;
		case 3:
			multi_register_write(ov9713_video_mode_3,
				ARRAY_SIZE(ov9713_video_mode_3), n);
			break;
		case 4:
			multi_register_write(ov9713_video_mode_4,
				ARRAY_SIZE(ov9713_video_mode_4), n);
			break;
		case 5:
			multi_register_write(ov9713_video_mode_5,
				ARRAY_SIZE(ov9713_video_mode_5), n);
			break;
		case 6:
			multi_register_write(ov9713_video_mode_6,
				ARRAY_SIZE(ov9713_video_mode_6), n);
			break;
		case 7:
			multi_register_write(ov9713_video_mode_7,
				ARRAY_SIZE(ov9713_video_mode_7), n);
			break;
		case 8:
			multi_register_write(ov9713_video_mode_8,
				ARRAY_SIZE(ov9713_video_mode_8), n);
			break;
		case 9:
			multi_register_write(ov9713_video_mode_9,
				ARRAY_SIZE(ov9713_video_mode_9), n);
			break;
		case 10:
			multi_register_write(ov9713_video_mode_10,
				ARRAY_SIZE(ov9713_video_mode_10), n);
			break;
		case 11:
			multi_register_write(ov9713_video_mode_11,
				ARRAY_SIZE(ov9713_video_mode_11), n);
			break;
		case 12:
			multi_register_write(ov9713_video_mode_12,
				ARRAY_SIZE(ov9713_video_mode_12), n);
			break;
		case 13:
			multi_register_write(ov9713_video_mode_13,
				ARRAY_SIZE(ov9713_video_mode_13), n);
			break;
		case 14:
			multi_register_write(ov9713_video_mode_14,
				ARRAY_SIZE(ov9713_video_mode_14), n);
			break;
		case 15:
			multi_register_write(ov9713_video_mode_15,
				ARRAY_SIZE(ov9713_video_mode_15), n);
			break;
		case 16:
			multi_register_write(ov9713_video_mode_16,
				ARRAY_SIZE(ov9713_video_mode_16), n);
			break;
		case 17:
			multi_register_write(ov9713_video_mode_17,
				ARRAY_SIZE(ov9713_video_mode_17), n);
			break;
		case 18:
			multi_register_write(ov9713_video_mode_18,
				ARRAY_SIZE(ov9713_video_mode_18), n);
			break;
		case 19:
			multi_register_write(ov9713_video_mode_19,
				ARRAY_SIZE(ov9713_video_mode_19), n);
			break;
		case 20:
			multi_register_write(ov9713_video_mode_20,
				ARRAY_SIZE(ov9713_video_mode_20), n);
			break;
		case 21:
			multi_register_write(ov9713_video_mode_21,
				ARRAY_SIZE(ov9713_video_mode_21), n);
			break;
		case 22:
			multi_register_write(ov9713_video_mode_22,
				ARRAY_SIZE(ov9713_video_mode_22), n);
			break;
		case 23:
			multi_register_write(ov9713_video_mode_23,
				ARRAY_SIZE(ov9713_video_mode_23), n);
			break;
		case 24:
			multi_register_write(ov9713_video_mode_24,
				ARRAY_SIZE(ov9713_video_mode_24), n);
			break;
		case 25:
			multi_register_write(ov9713_video_mode_25,
				ARRAY_SIZE(ov9713_video_mode_25), n);
			break;
		case 26:
			multi_register_write(ov9713_video_mode_26,
				ARRAY_SIZE(ov9713_video_mode_26), n);
			break;
		case 27:
			multi_register_write(ov9713_video_mode_27,
				ARRAY_SIZE(ov9713_video_mode_27), n);
			break;
		case 28:
			multi_register_write(ov9713_video_mode_28,
				ARRAY_SIZE(ov9713_video_mode_28), n);
			break;
		default:
	...
	*/
	...
	bool PS4EYECam::init_registers() {
		...
		multi_register_read(unknown30_reg_read, ARRAY_SIZE(unknown30_reg_read),
			0x30);
		...
		multi_register_read(unknow30_reg_read1, ARRAY_SIZE(unknow30_reg_read1),
			0x30);
	...
	bool PS4EYECam::init(uint32_t width, uint32_t height,
		...
		if (!init_registers())
		{
			...
			std::cout << "Initialization registers failed" << std::endl;
			return false;
		}
	...
	eyeframe * PS4EYECam::getLastVideoFramePointer() {
		...
		uint8_t *rowpointer;
		rowpointer =
			&(urb->frame_buffer[urb->frame_complete_ind * urb->frame_size]);
	...
	*/
	void PS4EYECam::dump_endpoint_comp(
		...
		debug("     USB 3.0 Endpoint Companion:\n"); debug("       bMaxBurst:        %d\n", ep_comp->bMaxBurst); debug("       bmAttributes:     0x%02x\n", ep_comp->bmAttributes); debug("       wBytesPerInterval: %d\n", ep_comp->wBytesPerInterval);
	}
	...
	void PS4EYECam::dump_endpoint(
		...
		debug("     Endpoint:\n"); debug("       bEndpointAddress: %02xh\n", endpoint->bEndpointAddress); debug("       bmAttributes:     %02xh\n", endpoint->bmAttributes); debug("       wMaxPacketSize:   %d\n", endpoint->wMaxPacketSize); debug("       bInterval:        %d\n", endpoint->bInterval); debug("       bRefresh:         %d\n", endpoint->bRefresh); debug("       bSynchAddress:    %d\n", endpoint->bSynchAddress);
	...
	int PS4EYECam::uvc_get_hue(int16_t *hue, uint8_t req_code) {
		...
		ret = libusb_control_transfer(handle_, REQ_TYPE_GET, req_code,
			UVC_PU_HUE_CONTROL << 8, 1 << 8, data, sizeof(data), 1000);
	...
	int PS4EYECam::uvc_set_hue(int16_t hue) {
		...
		ret = libusb_control_transfer(handle_,
			REQ_TYPE_SET, UVC_SET_CUR, UVC_PU_HUE_CONTROL << 8, 1 << 8, data,
			sizeof(data), 0);
	...
	int PS4EYECam::uvc_get_sharpness(uint16_t* sharpness, uint8_t req_code) {
		...
		ret = libusb_control_transfer(handle_,
			REQ_TYPE_GET, req_code, UVC_PU_SHARPNESS_CONTROL << 8, 1 << 8, data,
			sizeof(data), 0);
	...
	int PS4EYECam::uvc_set_sharpness(uint16_t sharpness) {
		...
		ret = libusb_control_transfer(handle_,
			REQ_TYPE_SET, UVC_SET_CUR, UVC_PU_SHARPNESS_CONTROL << 8, 1 << 8, data,
			sizeof(data), 0);
	...
	int PS4EYECam::uvc_get_brightness(int16_t* brightness, uint8_t req_code) {
		...
		ret = libusb_control_transfer(handle_,
			REQ_TYPE_GET, req_code, UVC_PU_BRIGHTNESS_CONTROL << 8, 1 << 8, data,
			sizeof(data), 0);
	...
	int PS4EYECam::uvc_set_brightness(int16_t brightness) {
		...
		ret = libusb_control_transfer(handle_,
			REQ_TYPE_SET, UVC_SET_CUR, UVC_PU_BRIGHTNESS_CONTROL << 8, 1 << 8, data,
			sizeof(data), 0);
	...
	int PS4EYECam::uvc_get_contrast(uint16_t* contrast, uint8_t req_code) {
		...
		ret = libusb_control_transfer(handle_,
			REQ_TYPE_GET, req_code, UVC_PU_CONTRAST_CONTROL << 8, 1 << 8, data,
			sizeof(data), 1000);
	...
	int PS4EYECam::uvc_set_contrast(uint16_t contrast) {
		...
		ret = libusb_control_transfer(handle_,
			REQ_TYPE_SET, UVC_SET_CUR, UVC_PU_CONTRAST_CONTROL << 8, 1 << 8, data,
			sizeof(data), 0);
	...
	int PS4EYECam::uvc_get_saturation(uint16_t* saturation, uint8_t req_code) {
		...
		ret = libusb_control_transfer(handle_,
			REQ_TYPE_GET, req_code, UVC_PU_SATURATION_CONTROL << 8, 1 << 8, data,
			sizeof(data), 0);
	...
	int PS4EYECam::uvc_set_saturation(uint16_t saturation) {
		...
		ret = libusb_control_transfer(handle_,
			REQ_TYPE_SET, UVC_SET_CUR, UVC_PU_SATURATION_CONTROL << 8, 1 << 8, data,
			sizeof(data), 0);
	...
	int PS4EYECam::uvc_get_white_balance_temperature(uint16_t* temperature,
		...
		ret = libusb_control_transfer(handle_,
			REQ_TYPE_GET, req_code, UVC_PU_WHITE_BALANCE_TEMPERATURE_CONTROL << 8,
	...
	int PS4EYECam::uvc_set_white_balance_temperature(uint16_t temperature) {
		...
		ret = libusb_control_transfer(handle_,
			REQ_TYPE_SET, UVC_SET_CUR, UVC_PU_WHITE_BALANCE_TEMPERATURE_CONTROL << 8,
			1 << 8, data, sizeof(data), 0);
	...
	int PS4EYECam::uvc_get_gain(uint16_t* gain, uint8_t req_code) {
		...
		ret = libusb_control_transfer(handle_,
			REQ_TYPE_GET, req_code, UVC_PU_GAIN_CONTROL << 8, 1 << 8, data,
			sizeof(data), 0);
	...
	int PS4EYECam::uvc_set_gain(uint16_t gain) {
		...
		ret = libusb_control_transfer(handle_,
			REQ_TYPE_SET, UVC_SET_CUR, UVC_PU_GAIN_CONTROL << 8, 1 << 8, data,
			sizeof(data), 0);
	...
	int PS4EYECam::uvc_get_power_line_frequency(uint8_t* power_line_frequency,
		...
		ret = libusb_control_transfer(handle_,
			REQ_TYPE_GET, req_code, UVC_PU_POWER_LINE_FREQUENCY_CONTROL << 8, 1 << 8,
			data, sizeof(data), 0);
	...
	int PS4EYECam::uvc_set_power_line_frequency(uint8_t power_line_frequency) {
		...
		ret = libusb_control_transfer(handle_,
			REQ_TYPE_SET, UVC_SET_CUR, UVC_PU_POWER_LINE_FREQUENCY_CONTROL << 8, 1 << 8,
			data, sizeof(data), 0);
	...
	int PS4EYECam::uvc_set_video_mode(uint8_t mode, uint8_t fps) {
		...
		case 0:        //3448x808
			...
			ctrl.dwMaxPayloadTransferSize = 0;//ISO_PACKET_SIZE;
			switch (frame_rate) {
			case 60:
		...
		case 1:        //1748x408
			...
			ctrl.bFrameIndex = 2;
			ctrl.dwMaxVideoFrameSize = frame_size;
			ctrl.dwMaxPayloadTransferSize = ISO_PACKET_SIZE;
			switch (frame_rate) {
			case 120:
		...
		/*
		err = libusb_control_transfer(handle_, 0x21, 0x01, 0x0100, 0x0001, buf, len,
		0);

		err = libusb_control_transfer(handle_, 0xa1, 0x81, 0x0100, 0x0001, buf, len,
		0);

		err = libusb_control_transfer(handle_, 0x21, 0x01, 0x0200, 0x0001, buf, len,
		0);
		*/
		if (err == len) {
	...
	int PS4EYECam::uvc_show_video_mode() {
		...
		/* do the transfer */
		err = libusb_control_transfer(handle_, 0xA1, 0x81, (1 << 8), 1, buf, len,
			0);
		if (err == len) {
			...
			debug("bmHint: %04x\n", ctrl.bmHint); debug("bFormatIndex: %d\n", ctrl.bFormatIndex); debug("bFrameIndex: %d\n", ctrl.bFrameIndex); debug("dwFrameInterval: %u\n", ctrl.dwFrameInterval); debug("wKeyFrameRate: %d\n", ctrl.wKeyFrameRate); debug("wPFrameRate: %d\n", ctrl.wPFrameRate); debug("wCompQuality: %d\n", ctrl.wCompQuality); debug("wCompWindowSize: %d\n", ctrl.wCompWindowSize); debug("wDelay: %d\n", ctrl.wDelay); debug("dwMaxVideoFrameSize: %u\n", ctrl.dwMaxVideoFrameSize); debug("dwMaxPayloadTransferSize: %u\n", ctrl.dwMaxPayloadTransferSize); debug("bInterfaceNumber: %d\n", ctrl.bInterfaceNumber);
	...
	bool PS4EYECam::open_usb() {
		...
		//set configuration 1
		//	res = libusb_set_configuration(handle_, 0);
	...
	*/
	void LIBUSB_CALL PS4EYECam::ct_done(struct libusb_transfer * transfer) {
		...
		if (cam->control_wLength != transfer->actual_length) {
			debug(
				"phase 2 read failed received %d bytes instead of %d\n",
	...
	*/
	void PS4EYECam::submit_controlTransfer(uint8_t bmRequestType, uint8_t bRequest,
		uint16_t wValue, uint16_t wIndex, uint16_t wLength, uint8_t *buffer) {
		libusb_fill_control_setup(buffer, bmRequestType, bRequest, wValue, wIndex,
			wLength);
		libusb_fill_control_transfer(control_transfer, handle_, buffer, ct_done,
			NULL, 1000);
	...
	void PS4EYECam::firmware_upload() {
		if (!firmwareisloaded) {
			...
			ifstream firmware(firmware_path.c_str(),
			...
			if (firmware.is_open()) {
				...
				for (uint32_t pos = 0; pos < length; pos += CHUNK_SIZE)
				{
					...
					submitAndWait_controlTransfer(0x40, 0x0, value, index, size,
						chunk);
